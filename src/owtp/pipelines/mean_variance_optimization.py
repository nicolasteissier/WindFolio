...
class MeanVariancePortfolioOptimizer:
    ...
    def __init__(self, start: pd.Timestamp, end: pd.Timestamp, target: Literal["paths", "paths_local"] = "paths_local"):
        ...
        self.covariance_csv_path = Path(self.config[target]['processed_data']) / "csv" / "covariance_matrix" / f"{self.file_name}.csv"
        self.covariance_parquet_path = Path(self.config[target]['processed_data']) / "parquet" / "covariance_matrix" / f"{self.file_name}.parquet"
...
if __name__ == "__main__":
    ...
    for window in windows:
        ...
        for lambda_risk in lambda_values:
            ...
            optimizer.save_weights(weights_cont, weights_int, results, window_suffix=window_suffix, suffix=suffix, verbose=True)
